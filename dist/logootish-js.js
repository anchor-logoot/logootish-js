...
class InsertionEvent {
    constructor(body, left, right, rclk, known_position) {
        this.type = EventType.INSERTION;
        this.body = '';
        this.start = undefined;
        this.known_position = undefined;
        this.rclk = new LogootInt();
        this.last = undefined;
        this.next = undefined;
    ...
    static fromJSON(eventnode) {
        return new InsertionEvent(eventnode.body, LogootPosition.fromJSON(eventnode.start), undefined, LogootInt.fromJSON(eventnode.rclk));
    }
    toJSON() {
        return {
            body: this.body,
...
class Document {
    constructor(send, insertLocal, removeLocal) {
        ...
        this.pending_events = [];
        this.vector_clock = new LogootInt();
    ...
    insert(position, text) {
        debug_1.debug.debug('INSERT', position, text);
        ...
        if (lesser && lesser.data.length + lesser.data.known_position > position) {
            left_position = lesser.data.start.offsetLowest(position - lesser.data.known_position);
        ...
        const event = new InsertionEvent(text, left_position, right_position, this.vector_clock, position);
    ...
    remove(position, length) {
        debug_1.debug.debug('REMOVE', position, length);
    ...
    _mergeNode(bst, nstart, length, resolveConflict, addNode, informRemoval) {
                ...
                if (result < 1) {
                    if (result < 0) {
                        if (conflict.whole_node) {
                            ...
                            if (clip_nstart) {
                                n.length = new LogootInt(start.l(level)).sub(n.start.l(level)).js_int;
                            ...
                            if (clip_nend) {
                                endnode.length = new LogootInt(n_end_old.l(level)).sub(end.l(level)).js_int;
                                if (endnode.length > 0) {
                                    addNode(endnode);
                                }
                            }
        ...
        if (lesser) {
            ...
            if (lesser.start.levels < nstart.levels) {
                positions.push(new LogootInt(nstart.l(lesser.start.levels)).sub(lesser.start.l(lesser.start.levels)).js_int);
            }
        ...
        skip_ranges.forEach((skip_range) => {
            ...
            const offset = new LogootInt(last_end.l(level)).sub(nstart.l(level))
                .js_int;
            const node = Object.assign(new LogootNode(), {
                offset
            });
    ...
    remoteInsert(event_contents) {
        const { body, start: nstart, rclk: this_rclk } = InsertionEvent.fromJSON(event_contents);
        debug_1.debug.debug('REMOTE INSERT', body, nstart.toString(), this_rclk.toString());
        if (this_rclk.cmp(this.vector_clock) > 0) {
            this.vector_clock = this_rclk;
            debug_1.debug.info('Fast-forward vector clock to', JSON.stringify(this_rclk));
        }
        const nodes = this._mergeNode(this.logoot_bst, nstart, body.length, (node, conflict, lesser) => {
        ...
        utils_1.arraymap(nodes, (node) => {
            ...
            return this._mergeNode(this.removal_bst, node.start, node.length, (node) => {
                if (node.rclk.cmp(this_rclk) < 0) {
                    return 0;
                }
                return 1;
    ...
    remoteRemove(event_contents) {
        ...
        removals.forEach((r) => {
            ...
            debug_1.debug.debug('REMOTE REMOVE', start.toString(), r.length, rclk.toString());
            const nodes = this._mergeNode(this.logoot_bst, start, r.length, (node) => {
            ...
            nodes.forEach((n) => {
                const length = new LogootInt(n.end.l(level)).sub(last_end.l(level))
                    .js_int;
                const nodes = this._mergeNode(this.removal_bst, last_end, length, (node) => {
                    if (node.rclk.cmp(rclk) < 0) {
                        return -1;
                    }
                    return 1;
